...
public class HLUtils {
	...
	public static PageInfo<?> findByHighLight(ElasticsearchTemplate elasticsearchTemplate, Class<?> clazz, Integer page,
		...
		final Pageable pageable = PageRequest.of(page - 1, rows, Sort.by(Sort.Direction.DESC, sortField));
		//查询对象
		...
		pi.setNextPage(page<pages?page+1:pages);
		pi.setPages(pages);
